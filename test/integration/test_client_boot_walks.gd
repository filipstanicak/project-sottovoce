...
# ------------------------------------------------- traverse is not a sprint --


func test_holding_run_and_pressing_traverse_does_not_sprint() -> void:
	# **THE OWNER RAN AT A WALL AND ARRIVED IN A SPRINT.** GDD-02 §1.3 gives the
	# pad a second sprint route — "L2 full + A" — but `INPUT-TRAVERSE` is `Space`
	# on a keyboard, so a combo that asked only "is traverse held" made
	# Shift + Space sprint. It predates US-0090 and got easier to hit when Shift
	# started meaning Run.
	#
	# `PadSelection` knows whether a mapped pad holds the joypad bindings. Without
	# one, the combo does not exist — and CI has no pad, so this is the case that
	# runs here.
	Input.action_press(&"input_move_forward")
...
func test_traverse_alone_still_reaches_the_resolver() -> void:
	# The other half: gating the combo must not have cost the press its normal
	# job. Nothing is in front of the pawn here, so §7.2 case 7's silence is the
	# correct answer — what matters is that it stays in a locomotion state rather
