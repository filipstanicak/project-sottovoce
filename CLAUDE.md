...
1.6 s lock exists to charge for. The arc covers a constant **length of ground**
now, `HALFWIDTH × RANGE-MAX / distance`, so the ambiguity never shrinks as you
approach and the arc becomes a **whole ring at 4.0 m**. **No number was invented**:
the ring radius is emergent from the two tunables, and **invariant 33** pins it
outside kill reach. The falloff flattens as the arc opens, because a full ring has
no edges to fade.
