...
**6.0 m IS DERIVED AND NOT CHOSEN: IT IS `TUN-SUSPICION-OPEN-RADIUS`.** That is
already this game's answer to *how big is the space you are standing in* — the
alone-check's radius and the spatial-hash cell size — so the Compass stops pointing
exactly when your contract becomes one of the bodies around you. **Invariant 33 is
that equality**, amended the same day it was added, and it keeps its kill clause as
well: the second is implied by the first at the shipped ranges, by 0.2 m across two
independently tunable bands, which is too thin a margin to delete a rule over.

**AND THE CURVE IS THE PULSE CURVE'S SHAPE, WHICH NOTHING ASKED FOR AND IS THE
POINT.** It opens **4.8 degrees over the first fifteen metres of the approach and
138.8 over the last** — GDD-03 §8.2's *"long, flat approach followed by a sudden
...
degrees at half range, deleting the directional reading over most of a hunt;
