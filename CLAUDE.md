...
faster than the crowd; `--census` on a live server read **1.35 against 1.36 m/s** —
they already held `input_slow`. The tell was the **stops: 1.0 s** for the old bots
against **8.6–10.7 s** for the crowd; the civilian bots read **13.5 s**. The median
turn rate separates nobody, which the story says rather than tunes away.
