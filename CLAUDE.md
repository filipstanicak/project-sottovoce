...
### 2026-09-25 — Cinderfall catches everyone in it, US-0104

**THE REFERENCE'S CLOUD, BUILT.** Everyone inside it but the caster coughs, held in place, for as
long as it stands, late entrants included; the caster's own pursuer inside is stunned by the
caster; and the caster may kill there. ADR-0023, with the owner's answers of the same day.

**A SEVENTEENTH PAWN STATE, AND WHY NOT THE ONE THE ADR NAMED.** `Staggered` is your own failure
for a fixed time and `Stunned` is another player's stun for 4 s with the camera taken; a cough
lasts however long the cloud has left, which no fixed number can say. So `Choking` has no exit
of its own, the server releases it, and a predicting client only ever leaves late. Index 16,
appended, drawn in GDD-02 §3.

**NPCs ARE HELD TOO, OR THE STILL FIGURES WOULD BE THE PLAYERS.** `CrowdDirector` reads the same
clouds and a procession waits for its held member. Neither rig has clips, so both simply stand.

**AND ONE PLANT STAYED GREEN FIRST, WHICH IS WORTH KNOWING.** Removing the busy check for a
coughing player changed nothing a kill could reach, because there is no `Choking -> KillAnim`
edge. What it did change is `kill_ready`, the crosshair ring on that player's screen, and the
test asserts that now.

**OPEN, AND THE OWNER'S: `TUN-CINDERFALL-DURATION` 6.0 s is now how long a caught figure is
held**, longer than a pressed stun. The reference's cloud is about 4 s. ADR-0023 C.
...
**CHECKPOINT. MERGED SINCE THE LAST ONE: #232 (US-0101, the dressed district), #233
(`depends_on` guard), #235 (the portrait's colour), #236 (US-0102, civilian test
bots).** The owner's checkout is fast-forwarded by me after each of my merges now,
by the owner's request — `--ff-only`, clean `main` only.

**THE PORTRAIT WEARS THE PERSONA'S HUE (#235)**, and review made the palette own the
mapping rather than the body: `Palette.persona_hues` is data, falling back to each
`PersonaData.identity_hue`, and `Wardrobe` hands the HUD's own palette instance to
every body — so a colourblind palette (US-0083) moves the portrait and the district
together. A test holds both to one override no persona resource carries; the client
boot asserts the wardrobe and the HUD hold the same instance.
...
**39 %** of stroller walking with another *stroller* beside it and **58–64 %** on lanes
five or more strollers share — every one takes the *shortest* navmesh path. The first
figures (33–40 %, and 15 % for IDLE figures that do not walk) booked state changes to the
new state and counted processions as a stroller's company; the review of #237 found both,
and `tools/crowd_spread.gd` is now pure and tested. The four processions are 17 % of
...
**AND A RELEASE BLOCKER NOBODY TRACKED: NO NPC EVER SITS OR LEANS.** GDD-03 §6.3 rule
7 says clones must occupy every blend action a player can; nothing under
`scripts/systems/crowd/` touches a blend prop, lean spot or hiding spot, and
`TUN-CROWD-IDLE-GROUP-SIZE-MIN`/`-MAX` have no reader — the ninth field nobody reads.
A player blending on a bench is the only figure ever seen on one. The owner raised it
from the reference (NPCs sit on benches and stand in circles; players do the same to
blend). Flagged in GDD-03, the ROADMAP and US-0103.

**TWO OWNER DECISIONS ARRIVED FROM THE FIDELITY COMPARISON** — 14 (identification as
the player's act) and 15 (pursuer count), below with the rest.

**MISTAKES THIS STRETCH, STATED:** a count of 42 dangling `depends_on` ids that was a
substring grep (five was right); a claim that the old bots walked 57 % faster than the
crowd, made before measuring (they did not); a `taskkill /IM` that ended every Godot
process on the machine; and 5526904 pushed with the arch suite red, because my
command did not stop on the failure. Each corrected in the PR it happened in.

Local on `main` at 4bcda4b: arch 61 / 246 / 1 548; unit 218 / 1 855 / 1 847 / 32 736;
integration 33 / 244 / 243 / 681.

### 2026-09-24 — the test bots walk the crowd's walk, US-0102

**ASKED FROM THE CONTROLS: THE BOTS WALKED *"SEHR LINEAR UND EINHEITLICH"*.** With
the district dressed, a bot was the one figure of its colour nobody had to look at
twice, so identification could not be tested alone. A debug tool, not an opponent
(SCOPE_FENCE OUT #14): `tools/bot_civilian.gd` copies the crowd's choices from
their own sources — a uniform idle anchor, a path on the committed navmesh loaded
into a navigation map of the bot's own, blend-walk, an `NpcBrain` idle on arrival.

**MEASURED, AND THE MEASUREMENT CORRECTED ME.** I said the old bots walked 57 %
faster than the crowd; `--census` on a live server read **1.39 against 1.40 m/s** —
they already held `input_slow`. The tell was the **stops: 1.1 s** for the old bots
against **9.7–10.3 s** for the crowd; the civilian bots read **7.7 s**. And they turn
**more** than the crowd, 0.11 against 0.07 rad/s — the look keys sweep — which is
the next thing to close.

**AND THE REVIEW CORRECTED THE INSTRUMENT TWICE**, which is why those are the second
figures: the bot's own track came off the unsmoothed simulation while the others
came off drawn nodes, and a view gap did not end a stop, so two stops of 3 s read as
one of **6.0**. Every group goes through `BotCensus.drawn()` now.

**AND I KILLED EVERY GODOT PROCESS ON THE MACHINE ONCE, BY IMAGE NAME**, ending the
first measurement. Later runs stop their own PIDs only.
...
14. **SHOULD IDENTIFYING THE TARGET BE THE PLAYER'S ACT, AS IN THE REFERENCE?**
    Raised 2026-09-24 by the fidelity comparison the owner asked for (sources in the chat
    log, never here). The reference checks the game's one skill at the decisive moment
    twice over and ours does it for the player twice over: **its kill strikes the figure
    you chose**, and killing a civilian costs points *and* the contract, where
    `KillRules` picks the nearest *player* in reach and cone and an NPC can never be the
    target; and **its lock is a targeting act that confirms nothing**, where ours holds
    1.6 s and then outlines the true body for 1.5 s. With the district dressed
    (US-0101), *which persona* is read at a glance and *which of the twelve* is the
    whole game — and the server answers it on the last metres. **My recommendation is
    an ADR draft first**, costed, building nothing: a kill on a chosen figure, a wrong
    kill that costs, a lock that confirms nothing. It touches ADR-0013's kill rules and
    never-do #2's server-side judgement, which is why it is the owner's.
15. **HOW MANY PURSUERS MAY A PLAYER HAVE?** Raised by the same comparison. Ours is a
    Hamiltonian cycle — exactly one hunter each, always. The reference's is usually one,
    **none** for a player low on the scoreboard, and up to **four** (rare) for a leader,
    so it doubles as the catch-up lever. Its only `×2` is also catch-up — a **loss
...
| Pawn | **17 states declared** — **seventeen since US-0104's `Choking`** (ADR-0023): caught in somebody else's Cinderfall cloud, held until no cloud holds you, with no exit of its own because the cloud's remaining life is the duration; the server releases it. Before that: fifteen at M0, fourteen when **the Jog rung was removed in US-0090** (`Jog` is a retired ID absent from `ALL`), fifteen again since **ADR-0017 added `Staggered`** on 2026-09-01 for the three `TUN-*-STAGGER` rules that had nowhere to live, and **sixteen since US-0070's `Lunging`** — the committed dash, which is a state because 6 m of unpredicted movement is 6 m of rubber-band. **`ALL`'s order is the WIRE and is append-only** — `Snapshot.state_index` indexes into it, `Staggered` holds index 14, `Lunging` 15 and `Choking` 16, and `test_pawn_state_count.gd` refuses an insertion before either. Transition edges asserted against the normative diagram. **All seventeen implemented**: five locomotion + `Vault`, `Climb`, `Drop`, `KillAnim`, `StunAnim`, `Stunned`, `Blended`, `Dead`, `Respawning`, `Staggered`, `Lunging`, `Choking`. **`Blended` is declared, registered and UNREACHABLE** — nothing in `scripts/` transitions into it, because the entry condition is server-only knowledge the client cannot predict; `SYS-BLEND` writes `blend_state` on the context instead. So ANIMATION_SPEC §5's `Loco --> Blended` node can never fire. **`Dead` has an exit at last.** **`Staggered` is interruptible where the other three combat states are not** — never-do #13, since a whiffed lunger would otherwise be in a stunnable locomotion state — and **it keeps the camera**, because taking it is `Stunned`'s signature. **Every living state can reach `Dead` as of 2026-09-02** — `Drop` and `StunAnim` could not until then, and `test_every_living_state_can_reach_dead` asserts the property rather than the two names, because the machine gained two states in two days |
...
| Cinderfall | **SINCE ADR-0023 (US-0104) IT CATCHES EVERYONE IN IT BUT THE CASTER, AND THE CASTER MAY KILL THERE**, as the reference's does. `CinderfallCatch` re-decides every tick from the clouds, before any press is judged: the caster's own pursuer inside is stunned by the caster (`StunSystem.stun_from_cloud`, no tier floor, no swing), anyone else in locomotion is `Choking` until no cloud holds them, and late entrants are caught too. `CrowdDirector` holds NPCs inside with the same clouds, and a procession waits for its held member, so the still figures are not the players. `TUN-CINDERFALL-BLOCKS-KILL` and `-BLOCKS-LOS` are false and `-SUSPICION` is 0, neutralised rather than removed, and each switch still has a test that turns it back on. **Everything below in this row is the history.** **`ABIL-CINDERFALL` IS THE FIRST ABILITY THAT CHANGES THE WORLD** (US-0067). `CinderfallEffect` is eleven lines and calls `CinderfallVolumes.add`, which had been built, tested and callerless since US-0056. The cloud **forbids kill initiation inside it including the caster's own** — GDD-04 §3.1's *"design detail that carries the ability"*, and without it the dominant play is *cloud, then kill inside it*. **`end()` deliberately does not remove the cloud**: `expire` lags the burn-out by `RewindClamp.max_ticks()` so a kill validated in the past still meets a cloud that was up when it was pressed, and clearing it here would delete exactly that window. **Nothing draws it** — there is no VFX pass, so on a client a Cinderfall is an absence of information |
