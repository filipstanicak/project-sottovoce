...
**THE SINGLE-WRITER GUARD LISTED ITS MUTATORS AND NOW DERIVES THEM.** `GameState`
gained `adopt_match`, its third mutator, and `test_game_state_single_writer.gd`
held `replace` and `clear` in a two-entry constant — so the new one would have
joined the file without joining the guard. It reads every `-> void` function and
every `var` off `game_state.gd` now, with a premise that the derivation still
finds what has always been there. Falsified by a reader calling `adopt_match`,
which no list ever named.
...
31 995 assertions; arch 57 / 225 / 1 364; integration 33 / 244 /
243 passing / 189.1 s.
