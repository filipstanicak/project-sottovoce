...
**AND THE SPEC DISAGREES WITH ITSELF BY THE HEIGHT OF THE BAR.** §2 lists the timer
at the 48 px `DISPLAY` scale; §1.1 gives the element 48 px of height. The digits are
32 px, sized to the plate, and both documents say so rather than one of them
quietly losing.
...
| Strings | `data/strings/en.csv`, 106 keys, no user-facing literal anywhere else — the count read 56 from M0 until 2026-09-13 while the table grew past a hundred, which is this file's prose-count drift in a fourth row |
