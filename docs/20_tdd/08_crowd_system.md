...
## 7. Navmesh
The eight static bake constants live in `scripts/core/pawn_navigation.gd`
(`PawnNavigation`), shared by the district and sandbox. `MapBuild` applies them,
while layout clearance, steering and rescue read the same dimensions. This class
has no autoload dependency; the two map generators remain `-s` SceneTree tools.
The 2026-09-08 extraction changed ownership only, not values or generated bytes.
...
| `scripts/presentation/npc_view.gd` | Client-side view | **Exists.** One `PersonaBody` per NPC index — undressed until `Wardrobe` dresses it (US-0101), interpolated `TUN-NET-INTERP-BUFFER` in the past like `RemotePawns`. **Absence means "no update", never "gone"** — the opposite rule, because culling, rate LOD and the delta all omit NPCs a client must keep drawing. It culls by distance instead, one margin wider than the server. **The crowd wears its personas as of US-0101**: `Wardrobe` derives the roster from the seed `NET-S2C-MATCH-START` carries, with the server's own arguments (`CrowdRoster.PLAYABLE`, `TUN-LOBBY-MAX-PLAYERS`), and dresses nobody until it also holds `NET-S2C-LOBBY-STATE` — a coloured crowd around grey players names every player. This cell said *"no client is told it until `SYS-MATCH`"* until then; that had been false since 2026-09-13 |
