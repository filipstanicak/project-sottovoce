...
| The full match state machine including Final Contract | **Built 2026-09-08, seven of eight criteria as of 2026-09-13** — `MatchSystem` on `net_ticked`, `MatchClock` for the arithmetic, the cycle dealt at the countdown, `ticks_remaining` on the wire for the first time, and the **seed broadcast** (`NET-S2C-MATCH-START`, sent at `ACTIVE` and to every late joiner). **One is honestly open**: the "sixth phase" — the warning is an announcement, because `MatchPhase`'s ordinals are the wire; owner decision 10. **US-0079, and it IS M5 as of 2026-09-08.** The stated dependency on the lobby was re-examined rather than assumed and does not hold: every one of the story's criteria needs tick arithmetic, `Net.player_count()`, the already-built `ScoreEvent` multiplier or the already-M5 results screen, and the one real coupling — the COUNTDOWN trigger — is satisfied by a minimum-player rule the lobby later replaces. **US-0098 did not move with it**; the lobby is still M6. Decided |
