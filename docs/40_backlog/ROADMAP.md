...
| **M5** Depth | **3 abilities** (`ABIL-WHISPERBOLT` deferred 2026-08-27 to pay for escape — `SCOPE_FENCE.md` OUT #18), scoring with all bonuses, **the escape verb**, HUD, results screen, audio events, **the match state machine (US-0079, moved from M6 on 2026-09-08)** | US-0064–0077 less US-0068, plus US-0079, US-0097, and US-0100–0103 (persona deal, dressed district, civilian test bots, the crowd walking apart), US-0104 (Cinderfall as the reference's, ADR-0023) and US-0105 (the reference's HUD, ADR-0024) | — |
...
| | State |
|---|---|
| `ContractCycle` + repair on kill / death / disconnect / join | **Done** (US-0049, US-0050), and **`open()` has a caller under `scripts/` as of US-0079, 2026-09-08** — `MatchSystem.countdown_opened`. It waited for a COUNTDOWN phase that did not exist until then, so the live path was `report_join` alone: the cycle was a ring in **join order** and the first peer to connect always hunted the second, on every seed. **`report_death` has its first caller as of US-0060** |
...
| `BlendSystem`: pockets, groups, static props, concealment props | **Done, five of six** (US-0053, US-0054) — **HALF-TRUE FOR THE PROPS, FOUND 2026-09-25**: a player can sit or lean, and **no NPC ever does** (GDD-03 §6.3 rule 7, a release blocker, unbuilt and untracked until US-0103), so a player on a bench is the only figure ever seen on one. All four kinds are live; the twelve lean spots are derived from the stall table rather than hand-listed. The open criterion is *the occupant can see nothing*, which needs a client that renders a blend at all — US-0073, M5 |
...
| The full match state machine including Final Contract | **DONE 2026-09-13, eight of eight** — `MatchSystem` on `net_ticked`, `MatchClock` for the arithmetic, the cycle dealt at the countdown, `ticks_remaining` on the wire for the first time, the **seed broadcast** (`NET-S2C-MATCH-START`, sent at `ACTIVE` and to every late joiner), and the final warning as an **announcement** rather than a sixth phase — owner decision 10, taken 2026-09-13, which amended the criterion instead of adding a wire ordinal for something that changes no rules. **US-0079, and it IS M5 as of 2026-09-08.** The stated dependency on the lobby was re-examined rather than assumed and does not hold: every one of the story's criteria needs tick arithmetic, `Net.player_count()`, the already-built `ScoreEvent` multiplier or the already-M5 results screen, and the one real coupling — the COUNTDOWN trigger — is satisfied by a minimum-player rule the lobby later replaces. **US-0098 did not move with it**; the lobby is still M6. Decided |
