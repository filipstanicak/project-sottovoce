...
> **Status at the 2026-08-16 checkpoint: FOUR OF TEN STORIES DONE** — US-0039, US-0040, US-0042
> and US-0041 bar one blocked criterion. A headless server holds 78 walking NPCs on a baked
> navmesh, indexed by a shared grid.
>
> **What the crowd cannot do yet, said plainly.** Only **Stroll** and **Idle** are reachable:
> nothing assigns a formation slot (US-0043), nothing grants a gawk token or sets `startle_flag`
> (US-0044), there is **no LOD at all** (US-0045), and **no NPC is on the wire** (US-0030's four
> culling criteria and US-0031's two are still waiting on that). The crowd is real on the server
...
| `CrowdDirector` — group slots, four circuits, clone redistribution | **Half.** The class exists and ticks the crowd (US-0041); **slots, circuits and the 2 s rebalance are US-0043 and unstarted**, which is why `WALKING_GROUP` is a state nothing can enter |
