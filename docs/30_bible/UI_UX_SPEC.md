...
| Part | Spec |
|---|---|
| **Direction cone** | Filled arc, camera-relative. Half-width `TUN-COMPASS-CONE-HALFWIDTH` 12° **at maximum range**, widening to a full ring at `TUN-COMPASS-CONE-FULL-RADIUS` 20.0 m, which is `TUN-COMPASS-LOCK-RANGE` — `CompassMath.cone_halfwidth_for`, flat over the long approach and steep at the end. Soft gradient edges — **never a hard-edged needle**, because the visual must communicate imprecision — and the falloff **flattens as the arc opens**, because a full ring has no edges to fade. **The drawn bearing and the drawn width both chase the authoritative ones over `TUN-NET-INTERP-BUFFER`**: the wire quantises the bearing to 1.41° and the distance to 0.5 m, which drawn raw at 144 Hz is a staircase. The chase can only ever be **behind** the simulation, never ahead — §3.3's prohibition is on information that is *newer*. The camera's own yaw is **not** smoothed, so the cone tracks the mouse instantly |
