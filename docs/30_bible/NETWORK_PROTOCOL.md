...
| `NET-S2C-BLEND-DENIED` | E | Rel | on event | `reason:u8`. Requester only. **New US-0054.** `NET-C2S-BLEND-REQUEST` had no answer at all, so a press at an occupied hiding spot was indistinguishable from a broken button. **It carries a reason and nothing else** — no prop index, because the player is standing at the prop and already knows which one, and no occupant, because who is inside is exactly the identity `ASM-0030` exists to make expensive. Unlike a rejected kill the reason is safe to send: a full hay cart is level geometry with somebody in it, not a fact about a stranger |
