...
| `NET-C2S-SKIP-RESULTS` | E | Rel | once | — | Phase == RESULTS. **Channel corrected X → E on 2026-09-13**: the row said `SESSION` and the message was built on `EVENT` four days earlier, deliberately — it is a keypress-shaped request beside the other requests on `RequestWire`, not a session fact. The document drifted from the code, not the other way. Skip requires **unanimous** consent. **BUILT 2026-09-09** on `RequestWire`, the C2S doorway `net.gd` ran out of room for. It carries no payload: pressing is the yes |
...
| `NET-S2C-STUN-RESULT` | E | Rel | on event | `stunner:u8`, `target:u8`, `tick:u32`, `valid:bool`, `lockout_ticks:u16`. **Sent only to the stunner and the target. Built US-0061.** A landed stun reaches both, carrying the same `lockout_ticks` so each can plan around it. **A refusal reaches the stunner alone and carries `target` = 0** — telling the target that somebody tried to stun them would say *that player believes you are hunting them*, and a target slot that varied between refusals would let a prey read strangers apart. Every refusal costs the same `TUN-STUN-INVALID-STAGGER` and reports the same thing |
| `NET-S2C-ABILITY-STARTED` | E | Rel | on event | `peer:u8`, `ability:u8`, `origin:3×f32`, `dir:3×f32`, `tick:u32`. **Broadcast to all clients in tell radius — the legibility law on the wire**. **`dir`'s LENGTH is the granted distance, amended 2026-09-03** — it was a unit vector, and `origin + dir` is now exactly where the ability landed. Same convention as `NET-C2S-ABILITY-REQUEST`, whose direction length `AbilityRules.aim` reads as the *requested* distance; answering in a different convention left every client unable to say where an 8 m throw landed without re-deriving the server's clamp, which works only while a client cannot aim short. **The row's width is unchanged** |
| `NET-S2C-ABILITY-DENIED` | E | Rel | on event | `slot:u8`, `reason:u8`. Requester only |
| `NET-S2C-BLEND-DENIED` | E | Rel | on event | `reason:u8`. Requester only. **New US-0054.** `NET-C2S-BLEND-REQUEST` had no answer at all, so a press at an occupied hiding spot was indistinguishable from a broken button. **It carries a reason and nothing else** — no prop index, because the player is standing at the prop and already knows which one, and no occupant, because who is inside a prop is the identity of somebody the requester has no relationship with — never-do #12, not `ASM-0030`, which was void from 2026-09-22 (ADR-0021) and never governed *other* players' identities anyway. Unlike a rejected kill the reason is safe to send: a full hay cart is level geometry with somebody in it, not a fact about a stranger |
