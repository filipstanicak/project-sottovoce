...
| `EVT-CONTRACT-PORTRAIT-REVEALED` | `contract_portrait_revealed(persona: StringName)` | `PERSONA-*` | A Compass lock completes. **Its meaning narrowed on 2026-09-22 (ADR-0021)**: the persona is known from assignment, so this now says *a lock has completed for this contract* and the widget draws the `Identified` mark; the id is kept because ids are immutable | `CompassVm` |
