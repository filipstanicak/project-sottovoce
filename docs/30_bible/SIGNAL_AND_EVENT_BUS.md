...
| `EVT-CONTRACT-ASSIGNED` | `contract_assigned(reason: int)` | `START` `KILL` `RESPAWN` `REPAIR` `ESCAPE` | A new contract is issued. **`NET-S2C-CONTRACT-ASSIGNED` carries a contract slot and the bridge DROPS it** — a slot is a *peer identity*, which GDD-03 §8.5 still withholds, so the payload is the reason alone. **The contract's persona is a different question and is no longer withheld** (ADR-0021, 2026-09-22): it is shown from assignment, and it will ride this message once US-0078 gives players a persona. The row read *"forbids a client learning anything about its contract it has not earned by looking"* until then | `PortraitWidget` (**wired 2026-09-02**; it had subscribed since US-0073 and the signal had no emitter, so an earned portrait stayed lit across every reassignment), `CompassVm`, `Audio` |
| `EVT-CONTRACT-PORTRAIT-REVEALED` | `contract_portrait_revealed()` | — (no payload) | A Compass lock completes for this contract. **Narrowed and emptied on 2026-09-22 (ADR-0021)**: the persona is known from assignment, so this reports a *lock-completion fact* and carries nothing. It was declared `(persona: StringName)` with a `PERSONA-*` payload that no emitter ever filled — `HudBridge` sent `&""` and the widget ignored it — and a payload that says nothing is worse than none. The id is kept because ids are immutable | `PortraitWidget` |
