...
| 2026-08-03 | ASM-0030 | **VOID since 2026-09-22 (ADR-0021).** Contract portrait starts unknown and is filled permanently by completing a Compass lock — reconciles the brief's HUD element with the rule that a hunter is never told their contract's persona, and gives the lock a payoff worth its cost. |
...
| 2026-08-28 | — | **The ability pipeline exists and casts nothing.** US-0066, seven of eight. Five validations, integer-tick cooldowns started at activation, an aim that is **clamped rather than refused**, and a **reliable tell broadcast** — the one message in this game whose recipient list exists to include people rather than to withhold, because design law 3's failure mode is somebody not being told. **Cooldowns reset in `AbilitySystem.on_death` and not in `PawnContext.reset_for_spawn`**, where US-0062 expected them: a pawn is replayed by prediction. Closes US-0062's last open criterion. The eighth criterion — the locally predicted tell — needs a client that can cast, which is US-0071's. **And the four debug overlays now toggle with F3**, world tint included. |
| 2026-08-28 | — | **The scoring table is live: thirteen bonuses, judged at initiation.** US-0065. **`SYS-SCORE` is not a `GameSystem` — the fourth such call, for a fourth reason**: every bonus is judged at the `combat` stage, so a system at the `score` stage would answer one tick late. The two windows ride `SYS-SUSPICION` and `SYS-DETECTION`, which already read speed and sight. **TDD-10 §2.1's `PawnContext.speed_history` is amended to `MatchContext`** — a pawn is replayed by prediction, so a client would push twenty duplicate samples per reconciliation. **`SCORE-FOCUS` rides `can_lock`**, diverging from §2's literal *unbroken LOS* to unbroken *watching*: zero extra raycasts, and it closes US-0056's last open criterion. |
...
| 2026-09-25 | ADR-0022 | **Proposed: suspicion is what your contract sees.** Asked for by the owner after a recorded reference match, overruling owner decision 11's *wait for the playtest*. The reference keeps one meter per (hunter, contract), moved only by high-profile acts in the contract's sight and never recovering within a contract; ours is one global number read by seven consumers. The recording shows a hunter crossing roofs and running out of sight and still being paid the top rung, which ours would have paid 50 or nothing. The three rungs' values (300 / 200 / 50) were measured from the recording's results screen and already match. **Proposed, not accepted: nothing is built on it.** Of six sourcing questions the owner answered two the same day: any pursuer can be stunned (the stun floor goes), and no ability use is high-profile. |
| 2026-09-25 | ADR-0023 | **Cinderfall catches everyone in it, and the caster may kill there.** Owner decision: *"wie es im Original ist"*. The reference's cloud makes everyone around the caster cough, unable to run or fight back, and *cloud, then kill inside it* is its main use; the recording shows three such kills. The kill-block, the line-of-sight block and the +40 cost all go, a caught pursuer counts as stunned, and NPCs are caught too so the cloud does not name the players in it. Built by US-0104. |
| 2026-09-25 | ADR-0024 | **The HUD shows what the reference shows.** Owner decision: *"so treu wie möglich"*. Never-do #12 narrowed a second time: the global kill feed, player names in the HUD and a permanent own placement are lifted; names or markers over bodies stay banned, and the kill-cam is open. Also swept: GDD-06 §2.3's hit-direction row, a month stale after ADR-0013 set the warning's bearing to true. Built by US-0105; names wait on US-0078. |
| 2026-09-22 | ADR-0021 | **The hunter knows the face from the start.** Reported from the controls: *"Wie mein Opfer aussieht muss ich von Anfang an wissen … Wie macht das Original es?"* The reference shows the target's picture from assignment; ours (ASM-0030) showed `UNKNOWN` until a 20 m / 1.6 s Compass lock, on the argument that the persona collapses the crowd from 60–90 to 8–13. **That collapse is the game and M3 was built for it** — the clone floor and clone parity keep the 8–13 indistinguishable — so the hidden persona was a second protection against the same threat, and it made the one read the game is about (whose gait?) unlearnable until the lock did it. ASM-0030 is void; GDD-03 §8.5 loses its persona row; the lock keeps its 1.5 s reveal, `SCORE-FOCUS` and the `portrait_revealed` latch, now meaning *a lock completed*. **No wire change here**: the persona travels with the contract once players have one, which is US-0078's lobby. Owner decision 4 is closed with it. |
| 2026-09-15 | ADR-0020 | **Walking alone costs nothing.** Reported from the controls after the first timed match: *"Ich bin bereits exposed wenn ich mal alleine laufe ohne Gruppe."* It was designed: `TUN-SUSPICION-GAIN-OPEN` 6.0/s whenever no NPC stood within 6 m, at any speed, and with no decay while any source pays a walker reached **Exposed in 11.7 s**. The reference charges nothing for being alone — its meter moves on high-profile actions in the other player's sight, and walking is low profile wherever it happens. **Neutralised to 0 rather than removed**, on ADR-0013's `TUN-SCORE-RECKLESS` pattern: the ID, the field, the radius and the condition stay, and `SuspicionSources.of()` lists the `OPEN` bit only while its rate is above zero, so the HUD word and the number cannot disagree. Two things this raised are owner decisions rather than changes: the reference's meter is per relationship and sight-gated where ours is one number per player (decision 11), and the Final Contract's `×2` has no counterpart in the reference (decision 12). |
| 2026-09-04 | ADR-0019 | **A stunned pursuer fails the contract and is dealt a new one.** Reported from the controls as a statement of the reference's rule: *"if a pursuer gets stunned or the prey escapes, his contract is failed and he gets a new prey."* **Half of it was already true** — an escape has removed and reinserted the hunter since US-0097 — and a stun did nothing to the contract at all: the pursuer was frozen 4 s, exiled from that one target for 12 s, and **walked back to the same person**. `ContractSystem.report_stun` is `report_escape`'s own body with `Reason.STUNNED` on it, deliberately rather than a second route: the line that would drift first is `cycle.remember` before the removal, which is the only thing stopping the pursuer being handed straight back the person who stunned them. |
| 2026-09-04 | ADR-0019 | **`TUN-STUN-LOCKOUT` 12 s stays and its published reasoning is amended.** GDD-03 §10.2 justified the exile as *"without it a stun costs the hunter 4 seconds and they walk back"* — which this decision supersedes with a stronger version of the same argument. The exile is kept because it is a per-`(hunter, target)` rule that still binds if the cycle deals those two together again, and **removing it to tidy up would be the weakening never-do #13 forbids**. The prey is paid once: `TUN-SCORE-STUN` 200, never also `SCORE-ESCAPE`. |
| 2026-09-03 | ADR-0018 | **A Lunge into your pursuer stuns them, and design law 5 was wrong in every clause.** Owner decision 9. The reference's equivalent resolves against whoever it connects with, so half the ability — the defensive half — was absent. `TUN-SCORE-STUN` **100 → 200**, the reference's own number, and invariant 19 from `==` to `>`: a floor rather than a ratio, because a stun must still lose to a well-made kill. **Logged 2026-09-04** — this row and the index entry below were both missing, which is the third time this index has run short of the ADRs that exist. |
| 2026-09-01 | ADR-0017 | **The pawn state machine gains a fifteenth state, `Staggered`.** Three tunables have described it since M0 with nowhere to live (`TUN-KILL-CONTEST-STAGGER`, `TUN-STUN-INVALID-STAGGER`, `TUN-LUNGE-WHIFF-STAGGER`) and what was built instead is an initiation lockout, which blocks presses and leaves the player free to sprint away — so *"unable to act"* and *"flailing is strictly worse than doing nothing"* were both false. **The argument that decided it is that a lockout has no tell**: `state_id` is on the wire and `CombatLockouts` is on nobody's, so a prey who read a Lunge and watched the hunter whiff saw them stand up and walk away normally. Unblocks US-0070. |
| 2026-09-01 | ADR-0017 | **It is interruptible, and that is never-do #13 rather than a preference.** A whiffed lunger would otherwise be in a locomotion state, which is stunnable — a stagger stun could not reach would be a weakening dressed as an addition. The asymmetry with `StunAnim` is a rule: **you are protected while doing something, not while paying for having done it.** And it **keeps the camera**, because taking it is `Stunned`'s signature and nothing else may borrow it. |
| 2026-09-01 | ADR-0017 | **`TUN-KILL-CONTEST-STAGGER` models a mechanic the reference title does not have** — the contested kill and its bonus are the *sequel's*. Reported rather than acted on: it is a merged `TUN-` ID and a merged rule, and nothing in ADR-0017 depends on it. **The owner's to rule on.** |
| 2026-09-08 | — | **`SYS-MATCH` (US-0079) moves M6 -> M5.** Owner decision, taken after the dependency on US-0078 was re-examined criterion by criterion rather than assumed: every one of US-0079's criteria needs tick arithmetic, `Net.player_count()`, the already-built `ScoreEvent` multiplier or the already-M5 results screen (US-0077). The one real coupling is the COUNTDOWN **trigger**, satisfied by a minimum-player rule that the lobby later replaces rather than enables. **US-0098 does NOT move**: the M4 gate named four blockers for the playtest and the lobby is still M6. |
...
| 2026-08-27 | — | ~~**Open, priced, not decided**~~ **SETTLED 2026-09-08, see the row below: moving `SYS-MATCH` (US-0079) from M6 to M5** would let the first playtest run a milestone earlier, and is the only single-story lever that does. M5 already ships a **results screen** (US-0077), which nothing can open without a match end. US-0079's stated dependency on the lobby is worth re-examining rather than assumed. |
...
| 2026-09-06 | — | Owner resolves the US-0073 layout conflict in favour of UI_UX_SPEC: contract portrait top-left, tier bottom-left. The local readability pass follows that arrangement, keeps instruments in the specified centred 16:9 band, and starts debug overlays hidden (F3 restores them). Completing a lock shows a status placeholder until an authoritative persona source exists; it does not complete the portrait acceptance criterion. |
| 2026-09-08 | — | Split Snapshot values/API from SnapshotCodec; keep ordered encoding, decoding and quantised fingerprints together, preserving the static decode API and exact wire bytes. The extra navigation hop is preferable to separating the two wire definitions. Standalone refactor, not a US-0079 prerequisite: its phase/timer fields already exist. [Decision and verification](../40_backlog/SPLIT_THE_SNAPSHOT.md). |
| 2026-09-09 | US-0077 | Results use the delivered event log and pure ScoreFold queries; the live feed remains private. Missing placement stays unknown until the server supplies it (owner decision); no client tie-break, persona, passive or player name is invented. Skip remains a server-owned request, never a local dismissal. |
| 2026-09-13 | US-0079 | **Owner decision 10: the final warning is an announcement, not a sixth phase.** `MatchPhase.Phase` keeps five members, because their ordinals are the wire and a name for something that changes no rules would remap every client's phase. The story's first criterion is amended from *"All six phases"* to five phases and one announcement, and ticked; US-0079 is done, eight of eight. The warning stays `MatchClock.warning_at` + `MatchSystem.final_warning_announced`, as built on 2026-09-08. |
| 2026-09-09 | US-0077 | Supersedes the temporary unknown-placement choice above: use ScorePlacement from 22c80e7, including explicit joint winners. Send skip through RequestWire; close only on authoritative RESULTS time zero. Keep input suppressed until a real phase change, and omit the unreplicated vote tally. |
| 2026-09-24 | US-0101 | **The district wears its personas, by owner request from the controls.** Every persona body — player and clone — wears ART_BIBLE §3's identity hue on its clothing, and a client dresses **nobody** until it holds both the seed and the players' roster, because dressing either half alone names every player. The roster rides `NET-S2C-LOBBY-STATE` (the M0 row, without its unwritten `ready`), `PROTOCOL_VERSION` 4. `GreyboxBody` retired in favour of an undressed `PersonaBody`. |
...
| [ADR-0017](adr/ADR-0017-the-stagger-state.md) | The fifteenth pawn state — a failed action leaves you Staggered | Accepted | — | 2026-09-01 |
| [ADR-0018](adr/ADR-0018-the-prey-has-more-than-one-tooth.md) | The prey has more than one tooth — a Lunge stuns a pursuer; design law 5 revamped | Accepted | — | 2026-09-03 |
| [ADR-0019](adr/ADR-0019-a-stun-costs-the-contract.md) | A stun costs the pursuer the contract | Accepted | — | 2026-09-04 |
| [ADR-0020](adr/ADR-0020-walking-alone-costs-nothing.md) | Walking alone costs nothing — `TUN-SUSPICION-GAIN-OPEN` neutralised | Accepted | — | 2026-09-15 |
| [ADR-0021](adr/ADR-0021-the-hunter-knows-the-face.md) | The hunter knows the face from the start — ASM-0030 voided | Accepted | — | 2026-09-22 |
| [ADR-0022](adr/ADR-0022-suspicion-is-what-your-contract-sees.md) | Suspicion is what your contract sees | **Proposed** | ADR-0020, on acceptance | 2026-09-25 |
| [ADR-0023](adr/ADR-0023-cinderfall-catches-everyone-in-it.md) | Cinderfall catches everyone in it, and the caster may kill there | Accepted | — | 2026-09-25 |
| [ADR-0024](adr/ADR-0024-the-hud-shows-what-the-reference-shows.md) | The HUD shows what the reference shows — never-do #12 narrowed again | Accepted | — | 2026-09-25 |
