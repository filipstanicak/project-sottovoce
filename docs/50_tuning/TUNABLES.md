...
| `TUN-COMPASS-CONE-FULL-RADIUS` | 6.0 | m | 4–9 | Where the arc becomes a **whole ring** and the Compass stops saying *which way*, saying only *here, somewhere*. **Derived, not chosen: it equals `TUN-SUSPICION-OPEN-RADIUS`** — invariant 33 — because that is already this game's answer to *how big is the space you are standing in*, the radius the alone-check and the spatial-hash cell both use. Inside it your contract is one of the bodies around you, and the instrument has nothing left to add. Added 2026-08-27 for reference fidelity: *"when you're looking at a complete circle, you know that your target is within range"* — within **engagement** range, not touching. The first cut derived the radius from the half-width alone and closed the ring at 4.0 m, which was judged too tight at the controls. |
...
both ends: `ln(HALFWIDTH / 180) / ln(FULL-RADIUS / RANGE-MAX)`, which is **1.176** at the shipped
...
| Distance | 55 m | 40 m | 30 m | 20 m | 15 m | 10 m | 8 m | 6 m |
|---|---|---|---|---|---|---|---|---|
| Half-width | 13.3° | 19.3° | 27.1° | 43.7° | 61.3° | 98.6° | 128.3° | **180°** |
...
| 33 | `TUN-COMPASS-CONE-FULL-RADIUS == TUN-SUSPICION-OPEN-RADIUS` **and** `> TUN-KILL-RANGE + TUN-KILL-VALIDATION-GRACE` | **The arc must be a whole ring before it could name a body.** Added 2026-08-27 with the widening arc; **amended the same day**, when the ring radius stopped being derived from the half-width and became a tunable of its own. The first clause is where the 6.0 m comes from — the Compass stops pointing exactly when the game already considers your contract to be in the space you are standing in, which is invariant 1's shape applied to a radius. The second is the thesis: inside kill reach the instrument has stopped saying which way, so it can never point at the one body you are about to stab — identifying that body is what the 1.6 s lock is charging for. **The second clause is implied by the first at the shipped ranges** (the least legal open radius is 4.0 m and the largest legal validated reach 3.8 m) and is kept anyway, because that margin is 0.2 m across two independently tunable bands and it is the clause that would fire if either widened. |
