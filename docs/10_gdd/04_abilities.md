...
> **SUPERSEDED IN PART BY [ADR-0023](../00_meta/adr/ADR-0023-cinderfall-catches-everyone-in-it.md) (accepted 2026-09-25), NOT YET BUILT.** The burst will catch everyone in the radius but the caster, a caught pursuer counts as stunned, and the kill-block, the line-of-sight block and the suspicion cost all go: in the reference, *cloud, then kill inside it* is the ability's main use. This section is rewritten by US-0104 in the same commit as the behaviour, and until then it describes the build.
